...
class SchoolsOOP():
    ...
    def insert(self, d):
        ...
        # El punto debe estar DENTRO de un polígono de la capa stadiums (capa de polígonos)
        check_within_query = """
        ...
        if len(self.cur.fetchall()) == 0:
            return {'ok': False, 'message': 'Error: el punto insertado debe estar ubicada DENTRO de un polígono existente.', 'data': None}

        # Insertar
        insert_query = """
            INSERT INTO haikyuu.schools 
            (name, principal_name, students, volleyball_club_name, captain, geom)
            VALUES (%s, %s, %s, %s, %s, ST_SnapToGrid(ST_GeometryFromText(%s, %s), 0.0001))
            RETURNING id
        """
        values = [d['name'], d['principal_name'], d['students'], d['volleyball_club_name'], d['captain'], geom_wkt, EPSG_CODE]
        try:
    ...
    def update(self, d):
        ...
        check_within_query = """
            SELECT id FROM haikyuu.stadiums 
            WHERE ST_Within(ST_SnapToGrid(ST_GeometryFromText(%s, %s), 0.0001), geom)
        """
        self.cur.execute(check_within_query, [geom_wkt, EPSG_CODE])
        if len(self.cur.fetchall()) == 0:
            return {'ok': False, 'message': 'Error topológico: La escuela debe estar DENTRO de un estadio.', 'data': None}

        # Actualizar
        update_query = """
            UPDATE haikyuu.schools 
            SET name=%s, principal_name=%s, students=%s, volleyball_club_name=%s, captain=%s, 
                geom=ST_SnapToGrid(ST_GeometryFromText(%s, %s), 0.0001)
            WHERE id=%s
        """
        values = [d['name'], d['principal_name'], d['students'], d['volleyball_club_name'], d['captain'], geom_wkt, EPSG_CODE, row_id]
        try:
    ...
    def delete(self, d):
        try:
            self.cur.execute("DELETE FROM haikyuu.schools WHERE id=%s", [d['id']])
    ...
    def selectAsTuples(self, d):
        try:
            self.cur.execute("SELECT id, name, principal_name, students, volleyball_club_name, captain, ST_AsText(geom) FROM haikyuu.schools WHERE id=%s", [d['id']])
    ...
    def selectAsDicts(self, d):
        try:
            ...
            dict_cur.execute("SELECT id, name, principal_name, students, volleyball_club_name, captain, ST_AsText(geom) FROM haikyuu.schools WHERE id=%s", [d['id']])
